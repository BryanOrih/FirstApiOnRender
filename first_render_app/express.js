...
let countries = [
    ...
    { countryName: "France", capital: "Paris", population: 67060000, language: "French", currency: "Euro (€)", img: "" },
    { countryName: "China", capital: "Beijing", population: 1444216107, language: "Chinese", currency: "Renminbi (¥)", img: "" },
    { countryName: "India", capital: "New Delhi", population: 1393409038, language: "Hindi, English", currency: "Indian Rupee (₹)", img: "" },
    { countryName: "Russia", capital: "Moscow", population: 145912025, language: "Russian", currency: "Russian Ruble (₽)", img: "" },
...
app.get("/random", (req,res)=>{
    ...
    res.json(countries[random])
})
app.get("/search/:string", (req,res)=>{
    let searching = req.params["string"]
    let searched = countries.find(country => country.countryName.toLocaleLowerCase() == searching.toLocaleLowerCase())
    if(searched){
        res.json(searched)
    }else{
        res.send("The searched Country doesn't exist")
    }
})
app.get("/primary_Language/:language", (req,res)=>{
    let searching = req.params["language"]
    let searched = countries.filter(country => country.language.toLocaleLowerCase() == searching.toLocaleLowerCase())
    if(searched.length !== 0){
        res.json(searched)
    }else{
        res.send("No country with that primary language in our database")
    }
})

app.listen(port, ()=>{
    console.log("Successful at http://localhost:"+port)
})